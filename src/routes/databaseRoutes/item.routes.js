...
ItemRouter.get('/item', ItemFiltersMiddleware, async (req, res) => {
  try {
    ...
    const mongoSort = req.filters.sort;
    const limit = req.filters.limit;

    const data = await Item.find(mongoFilter).limit(limit).sort(mongoSort);
    const count = await Item.countDocuments(mongoFilter);

    return res.json({data, count, message: 'Товары успешно получены'});
  } catch (err) {
    console.log(err.message);
    return res
      .status(500)
      .json({data: [], message: 'Ошибка на сервере при получении товаров'});
  }
});
